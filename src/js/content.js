...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
  if (request.command === 'showArticleView') {
    document.head.innerHTML = '<meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no\" /><style>body, html { margin: 0; padding: 0 } body > div { margin: 0; padding: 15px; margin-bottom: 30px } img { max-width: 100% } iframe { width: 100%; position: fixed; left: 0; bottom: 0; background: #ededf3; border-width: 1px 0 0 0; border-style: solid; border-color: #c8c7cc; height: 45px; overflow: hidden }</style>';
